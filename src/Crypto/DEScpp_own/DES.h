...
//class _declspec(dllexport) Des   // "__declspec(dllexport)": 没有声明变量时忽略"CBtt"的左侧,可以使用 __declspec(dllexport) 关键字从 DLL 导出数据、函数、类或类成员函数。__declspec(dllexport) 会将导出指令添加到对象文件中
class  Des
{
public:
	Des();
