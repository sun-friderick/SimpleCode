...
::RTSPClientConnection(RTSPServer& ourServer, int clientSocket, struct sockaddr_in clientAddr)
  : GenericMediaServer::ClientConnection(ourServer, clientSocket, clientAddr),		//对RTSPServer的成员变量进行赋值：然后使用setBackgroundHandling函数设置incomingRequestHandler回调
...
/**
 *  首先还是分析Opinion，该命令请求的作用是客户端请求服务端支持哪些命令；
 *  Describe请求是得到会话描述信息，包括h264的sps，pps信息也可以在Describe的应答中发送；
 *  Setup命令是用来建立会话，服务端收到Setup请求后，建立会话，new 一个 RTSPClientSession 对象，该对象用来处理客户端的各种Rtsp命令请求；
 *      同时服务端保存会话Id和会话对象，每次可以从表中取出 RTSPClientSession 对象, 响应客户端的请求；
 *      在收到Setup命令后, 没有等到客户端的Play命令，就开始视频流；
 *  
 **/
...
::handleCmd_DESCRIBE(char const* urlPreSuffix, char const* urlSuffix, char const* fullRequestStr) {
  ...
  do {  //整理一下下RTSP地址
    ...
    //跟据流的名字查找 session(Server Session,即是 ServerMediaSession)
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
  ...
  do {
    ...
    if (parseSucceeded) {
      ...
      if (requestIncludedSessionId) {   //根据 sessionIdStr 查表，看该客户端的会话是否存在,若存在会话，调用noteLiveness函数
        clientSession = (RTSPServer::RTSPClientSession*)(fOurRTSPServer.lookupClientSession(sessionIdStr));
        if (clientSession != NULL) clientSession->noteLiveness();   //用来判断流是不是断开；这个相当重要，我们可以使用它判断网络是否断开，尤其在客户端可以使用这样的方法来判断网络是否断开，然后实现断网重连的功能。
      }
      ...
      //解析：收到客户端的 OPTIONS 请求
      if (strcmp(cmdName, "OPTIONS") == 0) {
	// If the "OPTIONS" command included a "Session:" id for a session that doesn't exist,
	// then treat this as an error:
    //根据是否包含"Session:" id 和 能否查找到 clientSession 判断是否已经存在会话, 若不存在会话，则应答 Session Not Found
	if (requestIncludedSessionId && clientSession == NULL) {
	  handleCmd_sessionNotFound();
	} else {
	  // Normal case:
	  handleCmd_OPTIONS();       //处理Opinion请求,构建应答包
	}
      } else if (urlPreSuffix[0] == '\0' && urlSuffix[0] == '*' && urlSuffix[1] == '\0') {
	// The special "*" URL means: an operation on the entire server.  This works only for GET_PARAMETER and SET_PARAMETER:
    
	if (strcmp(cmdName, "GET_PARAMETER") == 0) {    //解析： GET_PARAMETER 
	  handleCmd_GET_PARAMETER((char const*)fRequestBuffer);
	} else if (strcmp(cmdName, "SET_PARAMETER") == 0) { //解析： SET_PARAMETER 
	  handleCmd_SET_PARAMETER((char const*)fRequestBuffer);
	} else {
	  handleCmd_notSupported();
	}
      } else if (strcmp(cmdName, "DESCRIBE") == 0) {    //解析： DESCRIBE 
	handleCmd_DESCRIBE(urlPreSuffix, urlSuffix, (char const*)fRequestBuffer);        //收到客户端的Describe请求,处理该请求，构建应答包
      } else if (strcmp(cmdName, "SETUP") == 0) {       //解析： SETUP  //如果是第一次Setup，那么就需要调用 createNewClientSessionWithId 函数创建 clientSession 进行会话，然后将 fRequestBuffer 和 clientSession 关联起来
	...
	if (!requestIncludedSessionId) {
	  ...
	  if (authenticationOK("SETUP", urlTotalSuffix, (char const*)fRequestBuffer)) {
	    clientSession = (RTSPServer::RTSPClientSession*)fOurRTSPServer.createNewClientSessionWithId();  //调用 createNewClientSessionWithId 函数创建 clientSession 
	  } else {
	    areAuthenticated = False;
	  }
	}
	if (clientSession != NULL) {    //如果存在会话，直接查找原来的会话； 并构建Setup应答
...
/**
 *  类 RTSPClientSession 需要提供什么功能呢：
 *      需要监听客户端的rtsp请求并回应它，
 *      需要在DESCRIBE请求中返回所请求的流的信息，
 *      需要在SETUP请求中建立起RTP会话，
 *      需要在TEARDOWN请求中关闭RTP会话，
 *      等等;  
 **/
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  do {
      /**
       *  每个 ServerMediaSession 中至少要包含一个 ServerMediaSubsession 。
       *  一个 ServerMediaSession 对应一个媒体，可以认为是 Server 上的一个文件，或一个实时获取设备, 其包含的每个 ServerMediaSubSession 代表媒体中的一个Track。
       *  所以一个 ServerMediaSession 对应一个媒体，如果客户请求的媒体名相同，就使用已存在的 ServerMediaSession, 如果不同，就创建一个新的。
       *  一个流对应一个 StreamState,并且 StreamState 与 ServerMediaSubsession 相关，但代表的是动态的，而 ServerMediaSubsession 代表静态的。  
       *  
       *  fOurServer.lookupServerMediaSession(streamName) 中会在找不到同名 ServerMediaSession 时新建一个，
       *  代表一个RTP流的 ServerMediaSession 们是被 RTSPServer 管理的，而不是被 RTSPClientSession 拥有。为什么呢？
       *  因为 ServerMediaSession 代表的是一个静态的流，也就是可以从它里面获取一个流的各种信息，但不能获取传输状态。
       *  也就是不同客户 Client 可能连接到同一个流，所以 ServerMediaSession 应被 RTSPServer 所拥有。
       **/
...
RTSPServer::createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr) {
  return new RTSPClientConnection(*this, clientSocket, clientAddr);   	//对于每个新建立的客户端连接请求,new RTSPClientConnection的对象进行管理;
}

GenericMediaServer::ClientSession*
RTSPServer::createNewClientSession(u_int32_t sessionId) {   //根据 sessionId 创建 RTSPClientSession 会话
