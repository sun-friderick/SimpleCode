...
char leaFile[256] = "";
char logFile[256] = "";
char tempFile[256] = "/tmp/dualserver.tmp";
char filePATH[256] = "";
char htmlTitle[256] = "";
data71 lump;
...
const char sVersion[] = "Dual DHCP DNS Server Version 7.29 Linux Build 7035";
...
int DualDHCPAndDNSServer(int argc, char **argv)
{
	char logBuff[256];
				...
				if (dhcpService)
				{
					if (network.httpConn.ready && FD_ISSET(network.httpConn.sock, &readfds))
					{
						...
						if (req)
						{
							...
							{
								sprintf(logBuff, "Accept Failed, Error %u", errno);
					...
					if (network.dhcpListener.ready && FD_ISSET(network.dhcpListener.sock, &readfds) && gdmess(&dhcpr, 255) && sdmess(&dhcpr))
						alad(&dhcpr);
...
void *sendHTTP(void *lpParam)
{
	...
	while (kRunning && req->bytes > 0)
	{
		...
		if (select((req->sock + 1), NULL, &writefds1, NULL, &tv1) > 0)
		{
			if (req->bytes > 1024)
				sent  = send(req->sock, dp, 1024, MSG_NOSIGNAL);
			else
				sent  = send(req->sock, dp, req->bytes, MSG_NOSIGNAL);
...
void procTCP(data5 *req)
{
	...
	char logBuff[512];
	data7 *cache;
	//sprintf(logBuff, "Start %u=%u",dnsCache[currentInd].size(),dnsAge[currentInd].size());
	//logMess(logBuff, 2);
    //long page_size = sysconf(_SC_PAGE_SIZE);
    //long pages = sysconf(_SC_AVPHYS_PAGES);
    //debug(pages * page_size);
	...
	else
	{
		...
		{
			case DNS_TYPE_SOA:
				sprintf(req->tempname, "SOA for zone %s", req->query);
				addRRNone(req);
				addRRSOA(req);
				sendTCPmess(req);
			...
			case DNS_TYPE_NS:
				sprintf(req->tempname, "NS for zone %s", req->query);
				addRRNone(req);
				addRRNS(req);
				addRRAd(req);
				sendTCPmess(req);
			...
			case DNS_TYPE_IXFR:
				...
				default:
					sprintf(logBuff, "TCP Query %s not supported", strquery(req));
					logTCPMess(req, logBuff, 1);
					addRRError(req, RCODE_NOTIMPL);
					sendTCPmess(req);
		}
...
MYWORD sendTCPmess(data5 *req)
{
	...
	if (select((req->sock + 1), NULL, &writefds, NULL, &tv1) > 0)
	{
		...
		if (req->bytes == send(req->sock, req->raw, req->bytes, MSG_NOSIGNAL) && !errno)
			return 1;
	}
...
void *init(void *lparam)
{
	...
	{
		if (findServer(network.allServers, MAX_TCP_CLIENTS, cfig.zoneServers[0]) && findServer(network.allServers, MAX_TCP_CLIENTS, cfig.zoneServers[1]))
		{
...
//				}
			...
			pthread_attr_init(&attr);
			pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
			int errcode = pthread_create(&threadId, &attr, checkZone, &magin);
			pthread_attr_destroy(&attr);
		}
		else if (cfig.replication == 1)
		{
	...
	do
	{
		if (cfig.dhcpReplConn.ready && newNetwork.maxFD < cfig.dhcpReplConn.sock)
			...
			if (newNetwork.httpConn.loaded)
			{
				...
				else if (newNetwork.httpConn.server)
				{
					...
					else
					{
						...
						{
							bindfailed = true;
							sprintf(logBuff, "Http Interface %s TCP Port %u not available", IP2String(ipbuff, newNetwork.httpConn.server), newNetwork.httpConn.port);
							logMess(logBuff, 1);
						...
						else
						{
							...
							{
								bindfailed = true;
								sprintf(logBuff, "%s TCP Port %u Error on Listen", IP2String(ipbuff, newNetwork.httpConn.server), newNetwork.httpConn.port);
								logMess(logBuff, 1);
								closesocket(newNetwork.httpConn.sock);
							}
							else
			...
			for (int j = 0; j < MAX_SERVERS && newNetwork.listenServers[j]; j++)
			{
				...
				else
				{
					...
					if (nRet == SOCKET_ERROR)
					{
						...
						sprintf(logBuff, "Warning: %s UDP Port 53 already in use", IP2String(ipbuff, newNetwork.listenServers[j]));
						logDNSMess(logBuff, 1);
						continue;
					}
				...
				else
				{
					...
					else
					{
						...
						if (nRet == SOCKET_ERROR)
						{
							...
							sprintf(logBuff, "Warning: %s TCP Port 53 already in use", IP2String(ipbuff, newNetwork.listenServers[j]));
							logDNSMess(logBuff, 1);
						}
						else
						{
		...
		for (int i = 0; i < MAX_SERVERS && network.listenServers[i]; i++)
		{
			for (MYBYTE j = 0; j < MAX_SERVERS; j++)
			{
				if (!network.dhcpConn[j].server && !network.dnsUdpConn[j].server)
					break;
				else if (network.dhcpConn[j].server == network.listenServers[i] || network.dnsUdpConn[j].server == network.listenServers[i])
				{
...
bool getInterfaces(data1 *network)
{
	char logBuff[256];
	char ipbuff[32];
	...
	if (ioctl(cfig.fixedSocket, SIOCGIFCONF, &Ifc) >= 0)
	{
		...
		for (MYBYTE i = 0 ; i < numInterfaces; i++)
		{
			...
			{
				addServer(network->allServers, MAX_SERVERS, addr);

				if (!(flags & IFF_DYNAMIC))
				{
					MYBYTE k = addServer(network->staticServers, MAX_SERVERS, addr);

					if (k < MAX_SERVERS)
						network->staticMasks[k] = mask;
				}
			}
		}
    }

	if (!memcmp(cfig.oldservers, network->allServers, (MAX_SERVERS * sizeof(MYDWORD))))
		return false;
	...
	if (dnsService)
	{
		if (cfig.specifiedDnsServers[0])
		{
			...
			{
				if (!findServer(network->allServers, MAX_SERVERS, cfig.specifiedDnsServers[i]))
