...
int http_fetch(const char *url_tmp, char **fileBuf)
{
...
#ifdef NOT_USE_REALLOC
    printf("get http file len %d\n", contentLength);
#endif
    ...
    while (ret > 0) {
        ...
        if (ret > 0) {
             ...
             */
#ifdef NOT_USE_REALLOC
            contentLength = contentLength - ret;
#else
...
int makeSocket(const char *host)
{
    ...
    if (ret < 0) {
        ...
        if ((errno == EINPROGRESS) || (errno == EAGAIN)) {
            ...
            if ( select(sock + 1, NULL, &s, NULL, &t) > 0) {
                connected = 1;
            } else
                connected = 0;
        }
