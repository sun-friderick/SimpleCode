...
DynamicRTSPServer::createNew(UsageEnvironment& env, Port ourPort,
  ...
  /**
   *  调用DynamicRtspServer的构造函数，
   *        DynamicRtspServer 继承自 RTSPServerSupportingHTTPStreaming 类; 
   *        RTSPServerSupportingHTTPStreaming 类又继承自 RTSPServer 类； RTSPServerSupportingHTTPStreaming 类的主要作用是支持Http；
   **/
...
ServerMediaSession* DynamicRTSPServer::lookupServerMediaSession(char const* streamName, Boolean isFirstLookupInSession) {
  ...
  Boolean fileExists = fid != NULL;
   ...
   liveLogInfo(" ServerMediaSession* DynamicRTSPServer::fid[%p].\n\n", fid);
   liveLogInfo(" streamName[%s],isFirstLookupInSession[%d]::.\n\n", streamName, isFirstLookupInSession);

  // Next, check whether we already have a "ServerMediaSession" for this file:
   //根据 streamName 查找是否已经存在一个ServerMediaSession
  ServerMediaSession* sms = RTSPServer::lookupServerMediaSession(streamName);
  Boolean smsExists = sms != NULL;

   liveLogInfo(" ServerMediaSession* DynamicRTSPServer:: .sms[%p]\n\n", sms);
  // Handle the four possibilities for "fileExists" and "smsExists":
  if (!fileExists) {    //文件不存在
    if (smsExists) {    //删除 ServerMediaSession
      // "sms" was created for a file that no longer exists. Remove it:
      removeServerMediaSession(sms);
      sms = NULL;
    }
   liveLogInfo(" ServerMediaSession* DynamicRTSPServer::.\n\n");
    ...
    if (sms == NULL) {  //如果 ServerMediaSession 不存在,根据 streamName 新建一个 ServerMediaSession
      sms = createNewSMS(envir(), streamName, fid); 
      addServerMediaSession(sms);   //将 ServerMediaSession 和会话关联起来
    }
   liveLogInfo(" ServerMediaSession* DynamicRTSPServer::.\n\n");
...
//新建 ServerMediaSession 会话
static ServerMediaSession* createNewSMS(UsageEnvironment& env,
					char const* fileName, FILE* /*fid*/) {
   liveLogInfo("  ServerMediaSession* createNewSMS.\n\n");
