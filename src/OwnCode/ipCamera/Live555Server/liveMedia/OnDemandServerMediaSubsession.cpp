...
OnDemandServerMediaSubsession::sdpLines() {
 ...
 liveLogInfo (" OnDemandServerMediaSubsession::sdpLines  61616161 \n");
...
//从下面的这段代码中可以看到 rtsp, rtp, rtcp 的 socket 是不同的;
//MediaSession 包括多个 MediaSubSession, 每个 MediaSubSession 对应相应的 socket, source 和 sink, 形成一个数据流！
//一个MediaSession 包括多个连接，关联到多个socket
...
::getStreamParameters(unsigned clientSessionId,
  ...
  liveLogInfo(" OnDemandServerMediaSubsession::getStreamParameters  \n");
  ...
  if (fLastStreamToken != NULL && fReuseFirstSource) {
	...
	for (portNumBits serverPortNum = fInitialPortNum; ; ++serverPortNum) {
      ...
      //建立 RTCP socket, 其中RTCP的端口号是： RTCPport=RTPport || (RTPport + 1)
	  if (fMultiplexRTCPWithRTP) {
 ...
 *  RTP的建立流程: 
 *      在客户端发送Setup请求开始建立，
 *      客户端发送Setup请求时，会将RTP/RTCP的端口号告诉服务端，也会将Rtp over tcp还是udp的方式告诉到服务端，
 *      服务端收到Setup请求时，根据端口号建立socket，在收到客户端的Play命令时，启动流传输；
 **/
void OnDemandServerMediaSubsession::startStream(unsigned clientSessionId,
  ...
  liveLogInfo(" OnDemandServerMediaSubsession::startStream    \n");
...
FramedSource* OnDemandServerMediaSubsession::getStreamSource(void* streamToken) {
  if (streamToken == NULL) return NULL;
    ...
    liveLogInfo(" OnDemandServerMediaSubsession::getStreamSource 337 337 337  \n");
...
::getRTPSinkandRTCP(void* streamToken,
      ...
      liveLogInfo(" OnDemandServerMediaSubsession::getRTPSinkandRTCP  347 347 347\n");
  if (streamToken == NULL) {
    rtpSink = NULL;
    rtcp = NULL;
    return;
...
liveLogInfo(" OnDemandServerMediaSubsession::deleteStream  363 363 363 363 \n");
...
::getAuxSDPLine(RTPSink* rtpSink, FramedSource* /*inputSource*/) {
  liveLogInfo(" OnDemandServerMediaSubsession::getAuxSDPLine   \n");
...
void OnDemandServerMediaSubsession::seekStreamSource(FramedSource* /*inputSource*/,
    ...
    liveLogInfo(" OnDemandServerMediaSubsession::seekStreamSource    \n");
...
::createGroupsock(struct in_addr const& addr, Port port) {
   liveLogInfo( " OnDemandServerMediaSubsession::createGroupsock  \n");
...
::createRTCP(Groupsock* RTCPgs, unsigned totSessionBW, /* in kbps */
     ...
     liveLogInfo(" OnDemandServerMediaSubsession::createRTCP   \n");
  // Default implementation; may be redefined by subclasses:
