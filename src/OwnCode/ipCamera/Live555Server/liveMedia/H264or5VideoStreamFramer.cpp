...
unsigned H264or5VideoStreamParser::parse() {
  try {
	...
	liveLogInfo( "Parsed trailing %d-byte NAL-unit (nal_ref_idc: %d, nal_unit_type: %d (\"%s\"))\n",
	...
	liveLogInfo( "Parsed trailing %d-byte NAL-unit (nal_unit_type: %d (\"%s\"))\n",
...
#ifdef DEBUG
      u_int8_t nal_ref_idc = (fFirstByteOfNALUnit&0x60)>>5;
      liveLogInfo( "Parsed %d-byte NAL-unit (nal_ref_idc: %d, nal_unit_type: %d (\"%s\"))\n",
...
#ifdef DEBUG
      liveLogInfo( "Parsed %d-byte NAL-unit (nal_unit_type: %d (\"%s\"))\n",
...
#ifdef DEBUG
	  liveLogInfo( "Set frame rate to %f fps\n", usingSource()->fFrameRate);
#endif
	} else {
#ifdef DEBUG
	  liveLogInfo( "\tThis \"Video Parameter Set\" NAL unit contained no frame rate information, so we use a default frame rate of %f fps\n", usingSource()->fFrameRate);
#endif
	}
      }
...
#ifdef DEBUG
	  liveLogInfo( "\tThis \"Sequence Parameter Set\" NAL unit contained no frame rate information, so we use a default frame rate of %f fps\n", usingSource()->fFrameRate);
#endif
	}
      }
...
#ifdef DEBUG
    liveLogInfo( "H264or5VideoStreamParser::parse() EXCEPTION (This is normal behavior - *not* an error)\n");
#endif
