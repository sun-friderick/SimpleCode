...
ServerMediaSession* GenericMediaServer::lookupServerMediaSession(char const* streamName, Boolean /*isFirstLookupInSession*/) {
  liveLogInfo( " ServerMediaSession* GenericMediaServer::lookupServerMediaSession  .\n\n");
...
#ifdef DEBUG
  liveLogInfo("accept()ed connection from [%s] \n", AddressString(clientAddr).val()) ;
#endif
...
void GenericMediaServer::ClientSession::noteLiveness() {
  ...
  liveLogInfo( "Client session (id \"%08X\", stream name \"%s\"): Liveness indication\n", fOurSessionId, streamName);
#endif
...
void GenericMediaServer::ClientSession::livenessTimeoutTask(ClientSession* clientSession) {
  ...
  liveLogInfo( "Client session (id \"%08X\", stream name \"%s\") has timed out (due to inactivity)\n", clientSession->fOurSessionId, streamName);
#endif
  delete clientSession;
}
