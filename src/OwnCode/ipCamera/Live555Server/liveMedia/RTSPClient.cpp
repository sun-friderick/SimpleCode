...
int RTSPClient::openConnection() {
  do {
    ...
    fInputSocketNum = fOutputSocketNum = setupStreamSocket(envir(), 0);  //setupStreamSocket,建立的是TCP连接
    //fInputSocketNum = fOutputSocketNum = setupDatagramSocket(envir(), 0);   //改成 setupDatagramSocket 即可建立UDP连接
