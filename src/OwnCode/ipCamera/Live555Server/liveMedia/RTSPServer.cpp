...
::RTSPClientConnection(RTSPServer& ourServer, int clientSocket, struct sockaddr_in clientAddr)
  : GenericMediaServer::ClientConnection(ourServer, clientSocket, clientAddr),		//¶ÔRTSPServerµÄ³ÉÔ±±äÁ¿½øÐÐ¸³Öµ£ºÈ»ºóÊ¹ÓÃsetBackgroundHandlingº¯ÊýÉèÖÃincomingRequestHandler»Øµ÷
...
/**
 *  Ê×ÏÈ»¹ÊÇ·ÖÎöOpinion£¬¸ÃÃüÁîÇëÇóµÄ×÷ÓÃÊÇ¿Í»§¶ËÇëÇó·þÎñ¶ËÖ§³ÖÄÄÐ©ÃüÁî£»
 *  DescribeÇëÇóÊÇµÃµ½»á»°ÃèÊöÐÅÏ¢£¬°üÀ¨h264µÄsps£¬ppsÐÅÏ¢Ò²¿ÉÒÔÔÚDescribeµÄÓ¦´ðÖÐ·¢ËÍ£»
 *  SetupÃüÁîÊÇÓÃÀ´½¨Á¢»á»°£¬·þÎñ¶ËÊÕµ½SetupÇëÇóºó£¬½¨Á¢»á»°£¬new Ò»¸ö RTSPClientSession ¶ÔÏó£¬¸Ã¶ÔÏóÓÃÀ´´¦Àí¿Í»§¶ËµÄ¸÷ÖÖRtspÃüÁîÇëÇó£»
 *      Í¬Ê±·þÎñ¶Ë±£´æ»á»°IdºÍ»á»°¶ÔÏó£¬Ã¿´Î¿ÉÒÔ´Ó±íÖÐÈ¡³ö RTSPClientSession ¶ÔÏó, ÏìÓ¦¿Í»§¶ËµÄÇëÇó£»
 *      ÔÚÊÕµ½SetupÃüÁîºó, Ã»ÓÐµÈµ½¿Í»§¶ËµÄPlayÃüÁî£¬¾Í¿ªÊ¼ÊÓÆµÁ÷£»
 *  
 **/
 
//)·þÎñ¶Ë¶ÔOpinionÃüÁîµÄ´¦Àí
...
void RTSPServer::RTSPClientConnection::handleCmd_DESCRIBE(char const* urlPreSuffix, char const* urlSuffix, char const* fullRequestStr) 
{
  ...
  #ifdef DEBUG
      liveLogInfo( "RTSPServer::RTSPClientConnection::handleCmd_DESCRIBE , urlPreSuffix \"%s\", urlSuffix \"%s\", fullRequestStr \"%s\".\n\n", urlPreSuffix, urlSuffix, fullRequestStr);
  #endif

  do {  //ÕûÀíÒ»ÏÂÏÂRTSPµØÖ·
    ...
    #ifdef DEBUG
             liveLogInfo( "RTSPServer::RTSPClientConnection::handleCmd_DESCRIBE urlTotalSuffix[%s].\n\n", urlTotalSuffix);
     #endif
    //¼øÈ¨
    ...
    //¸ú¾ÝÁ÷µÄÃû×Ö²éÕÒ session(Server Session,¼´ÊÇ ServerMediaSession)
    session = fOurServer.lookupServerMediaSession(urlTotalSuffix);
    #ifdef DEBUG
             liveLogInfo( "RTSPServer::RTSPClientConnection::handleCmd_DESCRIBE session[%p].\n\n", session);
     #endif
    if (session == NULL) {
      handleCmd_notFound();
      break;
    ...
    #ifdef DEBUG
             liveLogInfo( "RTSPServer::RTSPClientConnection::handleCmd_DESCRIBE sdpDescription[%p].\n\n", sdpDescription);
    #endif
...
#ifdef DEBUG
  liveLogInfo( "Handled HTTP \"OPTIONS\" request\n");
#endif
...
#ifdef DEBUG
  liveLogInfo( "Handled HTTP \"GET\" request (client output socket: %d)\n", fClientOutputSocket);
#endif
...
#ifdef DEBUG
  liveLogInfo( "Handled HTTP \"POST\" request (client input socket: %d)\n", fClientInputSocket);
#endif
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
  ...
  do {
...
#ifdef DEBUG
      liveLogInfo( "RTSPClientConnection[%p]::handleRequestBytes() read %d new bytes (of %d); terminating connection!\n", this, newBytesRead, fRequestBufferBytesLeft);
#endif
      fIsActive = False;
      break;
    }
...
#ifdef DEBUG
    ptr[newBytesRead] = '\0';
    liveLogInfo( "RTSPClientConnection[%p]::handleRequestBytes() %s %d new bytes:%s\n", this, numBytesRemaining > 0 ? "processing" : "read", newBytesRead, ptr);
#endif
...
#ifdef DEBUG
	liveLogInfo( "Base64-decoded %d input bytes into %d new bytes:", numBytesToDecode, decodedSize);
    ...
    if (parseSucceeded) {
#ifdef DEBUG
      liveLogInfo(" parseRTSPRequestString() succeeded, returning cmdName \"%s\", urlPreSuffix \"%s\", urlSuffix \"%s\", CSeq \"%s\", Content-Length %u, with %ld bytes following the message.\n\n", cmdName, urlPreSuffix, urlSuffix, cseq, contentLength, ptr + newBytesRead - (tmpPtr + 2));
#endif
      ...
      if (requestIncludedSessionId) {   //¸ù¾Ý sessionIdStr ²é±í£¬¿´¸Ã¿Í»§¶ËµÄ»á»°ÊÇ·ñ´æÔÚ,Èô´æÔÚ»á»°£¬µ÷ÓÃnoteLivenessº¯Êý
        clientSession = (RTSPServer::RTSPClientSession*)(fOurRTSPServer.lookupClientSession(sessionIdStr));
        if (clientSession != NULL) clientSession->noteLiveness();   //ÓÃÀ´ÅÐ¶ÏÁ÷ÊÇ²»ÊÇ¶Ï¿ª£»Õâ¸öÏàµ±ÖØÒª£¬ÎÒÃÇ¿ÉÒÔÊ¹ÓÃËüÅÐ¶ÏÍøÂçÊÇ·ñ¶Ï¿ª£¬ÓÈÆäÔÚ¿Í»§¶Ë¿ÉÒÔÊ¹ÓÃÕâÑùµÄ·½·¨À´ÅÐ¶ÏÍøÂçÊÇ·ñ¶Ï¿ª£¬È»ºóÊµÏÖ¶ÏÍøÖØÁ¬µÄ¹¦ÄÜ¡£
      }
      ...
      //½âÎö£ºÊÕµ½¿Í»§¶ËµÄ OPTIONS ÇëÇó
      if (strcmp(cmdName, "OPTIONS") == 0) {
        	// If the "OPTIONS" command included a "Session:" id for a session that doesn't exist,
        	// then treat this as an error:
            //¸ù¾ÝÊÇ·ñ°üº¬"Session:" id ºÍ ÄÜ·ñ²éÕÒµ½ clientSession ÅÐ¶ÏÊÇ·ñÒÑ¾­´æÔÚ»á»°, Èô²»´æÔÚ»á»°£¬ÔòÓ¦´ð Session Not Found
        	if (requestIncludedSessionId && clientSession == NULL) {
        	  handleCmd_sessionNotFound();
        	} else {
        	  // Normal case:
        	  handleCmd_OPTIONS();       //´¦ÀíOpinionÇëÇó,¹¹½¨Ó¦´ð°ü
        	}
      } else if (urlPreSuffix[0] == '\0' && urlSuffix[0] == '*' && urlSuffix[1] == '\0') {
        	// The special "*" URL means: an operation on the entire server.  This works only for GET_PARAMETER and SET_PARAMETER:
            
        	if (strcmp(cmdName, "GET_PARAMETER") == 0) {    //½âÎö£º GET_PARAMETER 
        	  handleCmd_GET_PARAMETER((char const*)fRequestBuffer);
        	} else if (strcmp(cmdName, "SET_PARAMETER") == 0) { //½âÎö£º SET_PARAMETER 
        	  handleCmd_SET_PARAMETER((char const*)fRequestBuffer);
        	} else {
        	  handleCmd_notSupported();
        	}
      } else if (strcmp(cmdName, "DESCRIBE") == 0) {    //½âÎö£º DESCRIBE 
	     handleCmd_DESCRIBE(urlPreSuffix, urlSuffix, (char const*)fRequestBuffer);        //ÊÕµ½¿Í»§¶ËµÄDescribeÇëÇó,´¦Àí¸ÃÇëÇó£¬¹¹½¨Ó¦´ð°ü
      } else if (strcmp(cmdName, "SETUP") == 0) {       //½âÎö£º SETUP  //Èç¹ûÊÇµÚÒ»´ÎSetup£¬ÄÇÃ´¾ÍÐèÒªµ÷ÓÃ createNewClientSessionWithId º¯Êý´´½¨ clientSession ½øÐÐ»á»°£¬È»ºó½« fRequestBuffer ºÍ clientSession ¹ØÁªÆðÀ´
      	...
      	if (!requestIncludedSessionId) {
      	  ...
      	  if (authenticationOK("SETUP", urlTotalSuffix, (char const*)fRequestBuffer)) {
      	    clientSession = (RTSPServer::RTSPClientSession*)fOurRTSPServer.createNewClientSessionWithId();  //µ÷ÓÃ createNewClientSessionWithId º¯Êý´´½¨ clientSession 
      	  } else {
      	    areAuthenticated = False;
      	  }
      	}
      	if (clientSession != NULL) {    //Èç¹û´æÔÚ»á»°£¬Ö±½Ó²éÕÒÔ­À´µÄ»á»°£» ²¢¹¹½¨SetupÓ¦´ð
		 ...
		 || strcmp(cmdName, "SET_PARAMETER") == 0) {    //½âÎö£º TEARDOWN PLAY PAUSE
      	if (clientSession != NULL) {
      	  clientSession->handleCmd_withinSession(this, cmdName, urlPreSuffix, urlSuffix, (char const*)fRequestBuffer);
      	} else {
      	  handleCmd_sessionNotFound();
      	}
      } else if (strcmp(cmdName, "REGISTER") == 0) {    //½âÎö£º REGISTER
...
#ifdef DEBUG
      liveLogInfo( "parseRTSPRequestString() failed; checking now for HTTP commands (for RTSP-over-HTTP tunneling)...\n");
#endif
      ...
      *fLastCRLF = '\r';
      if (parseSucceeded) {
#ifdef DEBUG
	liveLogInfo( "parseHTTPRequestString() succeeded, returning cmdName \"%s\", urlSuffix \"%s\", sessionCookie \"%s\", acceptStr \"%s\"\n", cmdName, urlSuffix, sessionCookie, acceptStr);
#endif
...
#ifdef DEBUG
	liveLogInfo( "parseHTTPRequestString() failed!\n");
#endif
	handleCmd_bad();
      }
    }
...
#ifdef DEBUG
    liveLogInfo( "sending response: %s", fResponseBuffer);
#endif
...
/**
 *  Àà RTSPClientSession ÐèÒªÌá¹©Ê²Ã´¹¦ÄÜÄØ£º
 *      ÐèÒª¼àÌý¿Í»§¶ËµÄrtspÇëÇó²¢»ØÓ¦Ëü£¬
 *      ÐèÒªÔÚDESCRIBEÇëÇóÖÐ·µ»ØËùÇëÇóµÄÁ÷µÄÐÅÏ¢£¬
 *      ÐèÒªÔÚSETUPÇëÇóÖÐ½¨Á¢ÆðRTP»á»°£¬
 *      ÐèÒªÔÚTEARDOWNÇëÇóÖÐ¹Ø±ÕRTP»á»°£¬
 *      µÈµÈ;  
 **/
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  do {
      /**
       *  Ã¿¸ö ServerMediaSession ÖÐÖÁÉÙÒª°üº¬Ò»¸ö ServerMediaSubsession ¡£
       *  Ò»¸ö ServerMediaSession ¶ÔÓ¦Ò»¸öÃ½Ìå£¬¿ÉÒÔÈÏÎªÊÇ Server ÉÏµÄÒ»¸öÎÄ¼þ£¬»òÒ»¸öÊµÊ±»ñÈ¡Éè±¸, Æä°üº¬µÄÃ¿¸ö ServerMediaSubSession ´ú±íÃ½ÌåÖÐµÄÒ»¸öTrack¡£
       *  ËùÒÔÒ»¸ö ServerMediaSession ¶ÔÓ¦Ò»¸öÃ½Ìå£¬Èç¹û¿Í»§ÇëÇóµÄÃ½ÌåÃûÏàÍ¬£¬¾ÍÊ¹ÓÃÒÑ´æÔÚµÄ ServerMediaSession, Èç¹û²»Í¬£¬¾Í´´½¨Ò»¸öÐÂµÄ¡£
       *  Ò»¸öÁ÷¶ÔÓ¦Ò»¸ö StreamState,²¢ÇÒ StreamState Óë ServerMediaSubsession Ïà¹Ø£¬µ«´ú±íµÄÊÇ¶¯Ì¬µÄ£¬¶ø ServerMediaSubsession ´ú±í¾²Ì¬µÄ¡£  
       *  
       *  fOurServer.lookupServerMediaSession(streamName) ÖÐ»áÔÚÕÒ²»µ½Í¬Ãû ServerMediaSession Ê±ÐÂ½¨Ò»¸ö£¬
       *  ´ú±íÒ»¸öRTPÁ÷µÄ ServerMediaSession ÃÇÊÇ±» RTSPServer ¹ÜÀíµÄ£¬¶ø²»ÊÇ±» RTSPClientSession ÓµÓÐ¡£ÎªÊ²Ã´ÄØ£¿
       *  ÒòÎª ServerMediaSession ´ú±íµÄÊÇÒ»¸ö¾²Ì¬µÄÁ÷£¬Ò²¾ÍÊÇ¿ÉÒÔ´ÓËüÀïÃæ»ñÈ¡Ò»¸öÁ÷µÄ¸÷ÖÖÐÅÏ¢£¬µ«²»ÄÜ»ñÈ¡´«Êä×´Ì¬¡£
       *  Ò²¾ÍÊÇ²»Í¬¿Í»§ Client ¿ÉÄÜÁ¬½Óµ½Í¬Ò»¸öÁ÷£¬ËùÒÔ ServerMediaSession Ó¦±» RTSPServer ËùÓµÓÐ¡£
       **/
	...
	if (absStart != NULL) {
	  ...
	  if (!sawRangeHeader || startTimeIsNow) {
	    ...
	    if (rangeEnd > 0.0 && (rangeEnd+0.001) < duration) {
         ...
         //²éÕÒÁ÷
	    fStreamStates[i].subsession->seekStream(fOurSessionId, fStreamStates[i].streamToken, rangeStart, streamDuration, numBytes);
	  }
	}
      }
    }
...
RTSPServer::createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr) {
  return new RTSPClientConnection(*this, clientSocket, clientAddr);   	//¶ÔÓÚÃ¿¸öÐÂ½¨Á¢µÄ¿Í»§¶ËÁ¬½ÓÇëÇó,new RTSPClientConnectionµÄ¶ÔÏó½øÐÐ¹ÜÀí;
}

GenericMediaServer::ClientSession*
RTSPServer::createNewClientSession(u_int32_t sessionId) {   //¸ù¾Ý sessionId ´´½¨ RTSPClientSession »á»°
