...
void RTCPMemberDatabase::reapOldMembers(unsigned threshold) {
  ...
  do {
...
#ifdef DEBUG
      liveLogInfo( "reap: checking SSRC 0x%lx: %ld (threshold %d)\n", (unsigned long)key, timeCount, threshold);
#endif
    ...
    if (foundOldMember) {
#ifdef DEBUG
        liveLogInfo( "reap: removing SSRC 0x%x\n", oldSSRC);
#endif
...
#ifdef DEBUG
  liveLogInfo( "RTCPInstance[%p]::RTCPInstance()\n", this);
#endif
  ...
  do {
...
#ifdef DEBUG
      liveLogInfo( "rejected bad RTCP packet: header 0x%08x\n", rtcpHdr);
#endif
      break;
    }
...
#ifdef DEBUG
	  liveLogInfo( "APP (subtype 0x%02x)\n", subtype);
#endif
	  if (length < 4) {
#ifdef DEBUG
	    liveLogInfo( "\tError: No \"name\" field!\n");
#endif
	    break;
	  }
#ifdef DEBUG
	  liveLogInfo( "\tname:%c%c%c%c\n", pkt[0], pkt[1], pkt[2], pkt[3]);
#endif
	  u_int32_t nameBytes = (pkt[0]<<24)|(pkt[1]<<16)|(pkt[2]<<8)|(pkt[3]);
	  ADVANCE(4); // skip over "name", to the 'application-dependent data'
#ifdef DEBUG
	  liveLogInfo( "\tapplication-dependent data size: %d bytes\n", length);
#endif
	...
	// Other RTCP packet types that we don't yet handle:
	  ...
	  if (length >= 12 && pkt[4] == 'R' && pkt[5] == 'E' && pkt[6] == 'M' && pkt[7] == 'B') {
	    ...
	    liveLogInfo( "\tReceiver Estimated Max Bitrate (REMB): %g bps\n", remb);
	  }
#endif
	  subPacketOK = True;
	  break;
...
#ifdef DEBUG
      liveLogInfo( "validated RTCP subpacket: rc:%d, pt:%d, bytes remaining:%d, report sender SSRC:0x%08x\n", rc, pt, length, reportSenderSSRC);
#endif
...
#ifdef DEBUG
	liveLogInfo( "extraneous %d bytes at end of RTCP packet!\n", packetSize);
#endif
	break;
      }
...
#ifdef DEBUG
	liveLogInfo( "bad RTCP subpacket: header 0x%08x\n", rtcpHdr);
#endif
	break;
      }
    }
    ...
    if (!packetOK) {
#ifdef DEBUG
      liveLogInfo( "rejected bad RTCP subpacket: header 0x%08x\n", rtcpHdr);
#endif
      break;
    } else {
#ifdef DEBUG
      liveLogInfo( "validated entire RTCP packet\n");
#endif
    }
...
#ifdef DEBUG
  liveLogInfo( "sending RTCP packet\n");
