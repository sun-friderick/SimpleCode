...
{
  if (f != point->sorted_file && fwrite(&buf->number, sizeof(unsigned long), 1, f) == 0) {
    return 0;
  }
                      ...
                      unsigned long *record_count)
{
  ...
  return merge_sort(unsorted_file, sorted_file, read_record, write_record, compare_records, &rwc, sizeof(unsigned long) + max_record_size,  block_size, record_count);
}
