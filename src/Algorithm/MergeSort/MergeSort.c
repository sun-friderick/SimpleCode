    ...
    while (source_tape[1].count != 0L)  {
      ...
      while (source_tape[0].count != 0L)  {
        ...
        while (count[0] + count[1] != 0)  {
          unsigned select = count[0] == 0 ? 1 : count[1] == 0 ? 0 : compare(record[0], record[1], pointer) < 0 ? 0 : 1;
          if ((*write)(destination_tape[destination].fp, record[select],  pointer) == 0) {
            if (destination_tape[0].fp != sorted_file)
                fclose(destination_tape[0].fp);
      ...
      fclose(source_tape[0].fp);
      fclose(source_tape[1].fp);
      if (fflush(destination_tape[0].fp) == EOF || fflush(destination_tape[1].fp) == EOF) {
        if (destination_tape[0].fp != sorted_file)
            fclose(destination_tape[0].fp);
        fclose(destination_tape[1].fp);
        free(record[0]);
