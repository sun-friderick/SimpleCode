...
get_geometry(char *dev, int fd, int silent) {
	...
	size = 0;
	if (!silent)
	    yx_msg(_("Disk %s: cannot get size\n"), dev);
    }
    if (ioctl(fd, HDIO_GETGEO, &g)) {
	g.heads = g.sectors = g.cylinders = g.start = 0;
	if (!silent)
	    yx_msg(_("Disk %s: cannot get geometry\n"), dev);
    }
    R.heads = g.heads;
    R.sectors = g.sectors;
...
reread_disk_partition(char *dev, int fd) {
    yx_msg(_("Re-reading the partition table ...\n"));
    fflush(stdout);
    sync();
    sleep(3);			/* superfluous since 1.3.20 */

    if(reread_ioctl(fd) && is_blockdev(fd))
      yx_msg(_("The command to re-read the partition table failed\n"
	     "Reboot your system now, before using mkfs\n"));

    if (close(fd)) {
	perror(dev);
	yx_msg(_("Error closing %s\n"), dev);
    }
    yx_msg("\n");
}
...
out_partition_header(char *dev, int format, struct geometry G) {
    ...
    switch(format) {
      default:
	yx_msg(_("unimplemented format - using %s\n"),
	       G.cylindersize ? _("cylinders") : _("sectors"));
      case F_CYLINDER:
	if (G.cylindersize) {
	  yx_msg(_("Units = cylinders of %lu bytes, blocks of 1024 bytes"
		 ", counting from %d\n\n"),
		 G.cylindersize<<9, increment);
	    yx_msg(_("   Device Boot Start     End   #cyls   #blocks   Id  System\n"));
	    break;
	}
	/* fall through */
      case F_SECTOR:
	yx_msg(_("Units = sectors of 512 bytes, counting from %d\n\n"),
	       increment);
	yx_msg(_("   Device Boot    Start       End  #sectors  Id  System\n"));
	break;
      case F_BLOCK:
	yx_msg(_("Units = blocks of 1024 bytes, counting from %d\n\n"),
	       increment);
	yx_msg(_("   Device Boot   Start       End   #blocks   Id  System\n"));
	break;
      case F_MEGABYTE:
	yx_msg(_("Units = megabytes of 1048576 bytes, blocks of 1024 bytes"
	       ", counting from %d\n\n"), increment);
	yx_msg(_("   Device Boot Start   End     MB   #blocks   Id  System\n"));
	break;
    }
}
...
out_partition(char *dev, int format, struct part_desc *p,
    ...
    if(pno == linux_to_index(lpno, z))  /* was that us? */
      yx_msg("%s", partname(dev, lpno, 10));  /* yes */
    ...
    if (dump) {
	yx_msg(" start=%9lu", start);
	yx_msg(", size=%8lu", size);
	if (p->ptype == DOS_TYPE) {
	    yx_msg(", Id=%2x", p->p.sys_type);
	...
	if(a.s && !is_equal_chs(a, b))
	  yx_msg(_("\t\tstart: (c,h,s) expected (%ld,%ld,%ld) found (%ld,%ld,%ld)\n"),
	...
	bb = chs_to_longchs(b);
	if(a.s && !is_equal_chs(a, b))
	  yx_msg(_("\t\tend: (c,h,s) expected (%ld,%ld,%ld) found (%ld,%ld,%ld)\n"),
		 aa.c, aa.h, aa.s, bb.c, bb.h, bb.s);
	if(G.cylinders && G.cylinders < 1024 && bb.c > G.cylinders)
	  yx_msg(_("partition ends on cylinder %ld, beyond the end of the disk\n"),
	       bb.c);
    }
}
...
extended_partition(char *dev, int fd, struct part_desc *ep, struct disk_desc *z) {
	...
	if (DOS_extended) {
	    ...
	    yx_msg(_("Warning: shifted start of the extd partition from %ld to %ld\n"),
		   ep->start, start);
	    yx_msg(_("(For listing purposes only. Do not change its contents.)\n"));
	} else {
	    yx_msg(_("Warning: extended partition does not start at a "
		     "cylinder boundary.\n"));
	    yx_msg(_("DOS and Linux will interpret the contents differently.\n"));
	}
    }
	...
	if (pno+4 >= SIZE(z->partitions)) {
	    yx_msg(_("too many partitions - ignoring those past nr (%d)\n"),
		   pno-1);
	    break;
	}
...
bsd_partition(char *dev, int fd, struct part_desc *ep, struct disk_desc *z) {
	...
	while (bp - bp0 <= BSD_MAXPARTITIONS) {
		if (pno+1 >= SIZE(z->partitions)) {
			yx_msg(_("too many partitions - ignoring those "
			       "past nr (%d)\n"), pno-1);
			break;
		}
...
msdos_partition(char *dev, int fd, unsigned long start, struct disk_desc *z) {
	...
	yx_msg(_("detected Disk Manager - unable to handle that\n"));
	return 0;
    }
    { unsigned int sig = *(unsigned short *)(s->data + 2);
	  ...
	  yx_msg(_("DM6 signature found - giving up\n"));
	  return 0;
      }
    }
	...
	if (is_extended(partitions[i].p.sys_type)) {
		...
		yx_msg(_("strange..., an extended partition of size 0?\n"));
		continue;
	    }
	...
	if (!bsd_later && is_bsd(partitions[i].p.sys_type)) {
	    if (!partitions[i].size) {
		yx_msg(_("strange..., a BSD partition of size 0?\n"));
		continue;
	    }
...
get_partitions(char *dev, int fd, struct disk_desc *z) {
	...
	yx_msg(_(" %s: unrecognized partition\n"), dev);
	return;
    }
}
...
write_partitions(char *dev, int fd, struct disk_desc *z) {
    ...
    if (no_write) {
	yx_msg(_("-n flag was given: Nothing changed\n"));
	exit(0);
    }
...
get_ul(char *u, unsigned long *up, unsigned long def, int base) {
	...
	*up = strtoul(u, &nu, base);
	if (errno == ERANGE) {
	    yx_msg(_("number too big\n"));
	    return -1;
	}
	if (*nu) {
	    yx_msg(_("trailing junk after number\n"));
	    return -1;
	}
    } else
      *up = def;
...
read_partition_chain(char *dev, int interactive, struct part_desc *ep,
    ...
    int i, base;
		...
		yx_msg("&&& -> read_partition_chain\n");
    eob = 0;
    while (1) {
	base = z->partno;
	if (base+4 > SIZE(z->partitions)) {
	    yx_msg(_("too many partitions\n"));
	    break;
	}
	for (i=0; i<4; i++)
...
int disk_partition_create(char *dev, int cntOfPartitions, int *dividedPercentage, int systype)
{
	...
	if(!no_write && !no_reread)    {
		...
		if(reread_ioctl(fd)) {
			...
			if (!force) {
				yx_msg(_("Use the --force flag to overrule all checks.\n"));
				return 3;
			}
		} else
			warn(_("OK\n"));
	...
	//while(1) 
	{
		...
		if (!partitions_ok(z) && !force) {
		    if(!interactive)
		      ...
		      yx_msg(_("I don't like this - probably you should answer No\n"));
		}
	}

	if(write_partitions(dev, fd, z))
		yx_msg(_("Successfully wrote the new partition table\n\n"));
	else
		exit_status = 1;
